...
|---|---|---|---|
| `id` | `Int` | No | PK autoincremental (default 0 al insertar) |
...
| `fechaInicio` | `String?` | Sí | Fecha de inicio de la suscripción Premium (`yyyy-MM-dd`) |
| `fechaFin` | `String?` | Sí | Fecha de fin de la suscripción Premium (`yyyy-MM-dd`) |

Los campos `fechaInicio` y `fechaFin` se leen en `ProfileViewModel` para calcular los días restantes de suscripción Premium y mostrarlos en `ProfileScreen`.
...
Modelo usado por `SecureSessionManager` para representar la sesión persistida del usuario. No está anotado con `@Serializable` porque no se serializa a JSON; se almacena campo a campo en `SharedPreferences`.
...
| `foto` | `String?` | Sí | Imagen de la máquina en Base64 |
...
| `horaFin` | `String` | No | Hora de fin (`HH:mm`). Máximo 60 minutos después de `horaInicio` |
...
| `imagenUrl` | `String?` | Sí | Cadena Base64 de la imagen |
| `fechaSubida` | `String?` | Sí | Timestamp de creación |
| `visibilidad` | `Boolean` | No | `true` = visible en el catálogo |
| `tiempo` | `String?` | Sí | Tiempo de preparación en texto libre (p. ej., `"30 minutos"`) |
| `aceptado` | `Boolean` | No | `false` = pendiente de moderación por administrador |
